...
def get_detections_from_im(im_file, image_id):
    ...
    original_image= cv2.imread(im_file)
    ...
    with torch.no_grad():
        ...
        topk_per_image = 36 ##### following the standard caption works
        ...
        cls_boxes = cls_boxes.cpu().float().numpy()

        box_features = box_features[result_per_image[0][-1]]
        box_features = box_features.cpu().float().numpy()

    return {
        'image_id': image_id,
        'image_h': inputs['height'],
        'image_w': inputs['width'],
        'num_boxes' : box_features.shape[0],
        'boxes': base64.b64encode(cls_boxes),
        'features': base64.b64encode(box_features)
    }
